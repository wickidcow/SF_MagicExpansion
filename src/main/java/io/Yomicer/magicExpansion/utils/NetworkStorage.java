...
public final class NetworkStorage {
    ...
    /**
     * 虚空之触绑定到量子存储方块时，以"物品模板 + long 数量"的方式存入，
     * 突破 ItemStack 单次 int 上限（1.20.5+ 大数量构造校验）。
     * <p>
     * 通过反射在确保不超容量的前提下分批调用 tryInputItem，每次最多传入 int 上限数量，
     * 直至全部存入或存储方不再接受（残量返回给调用方）。
     *
     * @param location  量子存储方块坐标
    ...
    public static long storeToQuantumStorageBlock(@NotNull Location location, @NotNull ItemStack prototype, long outputAmount) {
        ...
        try {
            ...
            // 分批调用 tryInputItem，突破单次 int 上限；每批传入 min(剩余, int上限)
            ...
            while (remaining > 0) {
                ...
                long storedThisBatch = batch - notStored;
                if (storedThisBatch <= 0) {
                    break; // 存储方已满/不再接受，剩余留待下 tick
                }
